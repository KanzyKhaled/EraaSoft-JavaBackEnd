...
//    }
    ...
    @Override
    public StudentDto getStudentWithTeacherById(Long id) {
        Student student = studentDao.findByIdWithTeacher(id);
        if (student == null) {
            throw new RuntimeException("Student not found with id: " + id);
        }
        return StudentMapper.studentMapper.toDto(student);
    }

    @Override
    public List<StudentDto> getAllStudentsWithTeachers() {
        List<Student> students = studentDao.findAllWithTeachers();
        return StudentMapper.studentMapper.toDtoList(students);
    }
